...
import { Trophy, Star, Shield, Zap, Target, ArrowRight, LucideIcon } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { cn } from '@/lib/utils';

export function AwardsSelection() {
  const { 
    isAwardsPhase, awardFinalists, setAwardWinner, selectedAwards, finalizeSeason 
  } = useLeague();
  
  const [currentStep, setCurrentStep] = useState(0);
  const categories = Object.keys(awardFinalists);
  const currentCategory = categories[currentStep];
  const finalists = awardFinalists[currentCategory] || [];
  
  const handleNext = () => {
    if (selectedAwards[currentCategory]) {
        ...
        <div className="bg-stone-50/50 p-8">
            ...
            <div className="flex items-center gap-4 mb-2">
              ...
              <div>
                ...
                <DialogDescription className="text-stone-500 font-medium">
                  Select the most deserving stuffy for each category
                </DialogDescription>
              </div>
            </div>
              ...
              {categories.map((cat, idx) => (
                <div 
                  ...
                  className={cn(
                    ...
                    idx === currentStep ? "bg-stone-900 w-full" : (idx < currentStep ? "bg-stone-300" : "bg-stone-100")
                  )}
                />
              ))}
            </div>
          ...
          <DialogFooter className="mt-8 pt-6 border-t border-stone-100">
            {currentStep < categories.length ? (
              <Button 
                onClick={handleNext}
                disabled={!selectedAwards[currentCategory]}
...
function getRelevantStats(player: Player, type: string) {
  ...
  if (type === 'MVP' || type === 'OPOY') {
    if (player.position === 'QB') return [
      { label: 'Yds', value: s.passingYards },
      { label: 'TD', value: s.passingTds },
      { label: 'INT', value: s.interceptionsThrown }
    ];
    if (player.position === 'RB') return [
      { label: 'Yds', value: s.rushingYards },
      { label: 'TD', value: s.rushingTds },
      { label: 'Car', value: s.carries }
    ];
    return [
      { label: 'Yds', value: s.receivingYards },
  ...
  if (type === 'DPOY') {
    return [
      { label: 'Tkl', value: s.tackles },
      { label: 'Sck', value: s.sacks },
      { label: 'Int', value: s.interceptions },
      { label: 'PD', value: s.passDeflections }
    ];
  }
  if (type === 'STPOY') {
    return [
